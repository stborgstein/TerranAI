...
public class PlayerTutorial20379586 extends DefaultBWListener {
				...
				if (!game.isBuildable(checkTile, true)) {
					...
					for (Unit neutralUnit : game.neutral().getUnits()) {
						if (neutralUnit.getX() / 32 == tileX && neutralUnit.getY() / 32 == tileY) {
							...
							// If the position is buildable, check how many tiles available to build for
							// that position
							int right = tileX + 1;
							int down = tileY + 1;
							int maximum = 1;
							...
							while (game.isBuildable(explore, true)
									&& !(mapMatrix[explore.getX()][explore.getY()] == "M")
	...
	public void writeMatrixToFile() throws IOException {
		...
		for (int i = 0; i < game.mapHeight();) {
			for (int j = 0; j < game.mapWidth(); j++) {
				String content = mapMatrix[j][i] + " - ";
				bw.write(content);
				/*
				 * if(j == game.mapWidth() - 1) { System.out.println(' '); i++; j = 0; }
				 */
			}
